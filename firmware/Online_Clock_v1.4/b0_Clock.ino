    ...
    if (i == 1) {
      ...
      for (byte k = 0; k <= 13; k++) {  // Счётчик для каждого светодиода в 4-м сегменте
        if (digits[digit][k] == 1) {    // Если массив для цифр с подставленным временем "digit" и счётчиком "k" равен 1, то...
          ...
          if (Regim_Color == 2 && Flag_Each_Sigment == true) {
            leds[cursor].setHue(RndFourHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ПОДНЯТ флаг для разноцветных сегментов, то устанавливаем рандомный цвет для 4-го сегмента
          } else if (Regim_Color == 2 && Flag_Each_Sigment == false) {
            leds[cursor].setHue(RndAllSegmHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ОПУЩЕН флаг для разноцветных сегментов, то устанавливаем рандомный цвет на все сегменты
          } else {
      ...
      for (byte k = 0; k <= 13; k++) {  // Счётчик для каждого светодиода в 3-м сегменте
        if (digits[digit][k] == 1) {    // Если массив для цифр с подставленным временем "digit" и счётчиком "k" равен 1, то...
          ...
          if (Regim_Color == 2 && Flag_Each_Sigment == true) {
            leds[cursor].setHue(RndThreeHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ПОДНЯТ флаг для разноцветных сегментов, то устанавливаем рандомный цвет для 3-го сегмента
          } else if (Regim_Color == 2 && Flag_Each_Sigment == false) {
            leds[cursor].setHue(RndAllSegmHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ОПУЩЕН флаг для разноцветных сегментов, то устанавливаем рандомный цвет на все сегменты
          } else {
            leds[cursor] = ledColor;  // Иначе используем общую переменную для "Палитра цветов" или "Рандомные цвета" на все сегменты
      ...
      for (byte k = 0; k <= 13; k++) {  // Счётчик для каждого светодиода в 2-м сегменте
        if (digits[digit][k] == 1) {    // Если массив для цифр с подставленным временем "digit" и счётчиком "k" равен 1, то...
          ...
          if (Regim_Color == 2 && Flag_Each_Sigment == true) {
            leds[cursor].setHue(RndTwoHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ПОДНЯТ флаг для разноцветных сегментов, то устанавливаем рандомный цвет для 2-го сегмента
          } else if (Regim_Color == 2 && Flag_Each_Sigment == false) {
            leds[cursor].setHue(RndAllSegmHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ОПУЩЕН флаг для разноцветных сегментов, то устанавливаем рандомный цвет на все сегменты
          } else {
            leds[cursor] = ledColor;  // Иначе используем общую переменную для "Палитра цветов" или "Рандомные цвета" на все сегменты
      ...
      for (byte k = 0; k <= 13; k++) {
        if (digits[digit][k] == 1) {  // Если массив для цифр с подставленным временем "digit" и счётчиком "k" равен 1, то...
          ...
          if (Regim_Color == 2 && Flag_Each_Sigment == true) {
            leds[cursor].setHue(RndOneHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ПОДНЯТ флаг для разноцветных сегментов, то устанавливаем рандомный цвет для 1-го сегмента
          } else if (Regim_Color == 2 && Flag_Each_Sigment == false) {
            leds[cursor].setHue(RndAllSegmHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ОПУЩЕН флаг для разноцветных сегментов, то устанавливаем рандомный цвет на все сегменты
          } else {
            leds[cursor] = ledColor;  // Иначе используем общую переменную для "Палитра цветов" или "Рандомные цвета" на все сегменты
...
void TempToArray(int DataTempIn) {
  ...
  if (Regim_Color == 2 && Flag_Each_Sigment == true) {
    ...
    leds[29].setHue(RndAllSegmHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ОПУЩЕН флаг для разноцветных сегментов, то устанавливаем рандомный цвет на все сегменты
  } else {
  ...
  for (byte с = 0; с <= 13; с++) {  // Счётчик для каждого светодиода в 4-м сегменте
    if (symbols[с] == 1) {          // Ищем в первом массиве цифры "1" и...
      //Устанавливаем цвет в зависимости от режима и флага разноцветных сегментов в заданный курсором диапазоне (от 44 до 58 светодиод)
      if (Regim_Color == 2 && Flag_Each_Sigment == true) {
        leds[cursor].setHue(RndFourHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ПОДНЯТ флаг для разноцветных сегментов, то устанавливаем рандомный цвет для маленького знака "C" (Цельсия)
      } else if (Regim_Color == 2 && Flag_Each_Sigment == false) {
        leds[cursor].setHue(RndAllSegmHUE);  // Если Вкл. 2 режим (Рандомные цвета) и ОПУЩЕН флаг для разноцветных сегментов, то устанавливаем рандомный цвет на все сегменты
      } else {
        leds[cursor] = ledColor;  // Иначе используем общую переменную для "Палитра цветов" или "Рандомные цвета" на все сегменты
...
void Hourly_Signal() {
  ...
  if (minute == 59 && second == 59) {        // Проверяем время, если на часах 59 минут и 59 секунд, то...
