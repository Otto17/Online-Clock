...
void Random_Color() {
  // String r, g, b;       // Строковые переменные, куда будем получать цвета в HEX (всегда по 2 символа)
  // byte rR = random8();  // Генерируем рандомные 8-ми битные числа для каждого цвета
  // byte gR = random8();  // "random8()" входит в библиотеку "FastLED.h"
  // byte bR = random8();

  // //При генерации чисел от 0 до 15 (включительно) будет всего 1 символ, а нужно два, что бы в сумме было 6 символов (по 2 на цвет), исправим это...
  // //Если выпадает число меньше 16 (так как в HEX от 16 начинаются двухзначные числа), то сначала переменной "r" присваиваем символ нуля,
  // if (rR < 16) r = '0' + String(rR, HEX);  // а потом ещё прибавляем к символу нуля рандомное число, переконвертированное в строку. К примеру получили цифру "3", а на выходе будет HEX строка "03"
  // else r = String(rR, HEX);                // А если выпадает число от 16 до 255, то просто конвертируем в строку

  // if (gR < 16) g = '0' + String(gR, HEX);
  // else g = String(gR, HEX);

  // if (bR < 16) b = '0' + String(bR, HEX);
  // else b = String(bR, HEX);

  // String RandomColor = r + g + b;  // Складываем HEX строки (получаем 3 числа цвета в 6 символов одной строкой)

  // //Блок "String to CharArray*" для корректного перевода строки в массив символов и конвертации HEX массива символов (цвета) в unsigned long.
  // char Buf_HexRand[sizeof(RandomColor)];                      // Создаём буфер и вычисляем его размер
  // RandomColor.toCharArray(Buf_HexRand, sizeof(RandomColor));  // myString.toCharArray(buf, len) – записывает строку в массив – буфер Buf_HexRand (типа char []) с начала и до длины len.
  // ledColor = strtoul(RandomColor.c_str(), NULL, 16);          // Конвертируем (без остатка от преобразования - NULL) HEX массив символов с указанием на шестнадцатеричную систему счисления (цифра 16 в конце) и пишем наше число с цветом в глобальную переменную
