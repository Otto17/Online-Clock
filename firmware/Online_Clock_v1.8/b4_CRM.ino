...
void myLoopRun() {
  ...
  time_temp = crm.var("TimeTemp").toInt() * 1000U;  // Получаем время отображения (в секундах) температуры на часах с WEB интерфейса, при нажатии кнопки "SET" на часах. Конвертируем в миллисекунды (количество миллисекунд = количество минут * 60000), (U - unsigned, выделяем память для беззнаковой константы)
...
*/
  ...
  myLoop.attach_ms(1000, myLoopRun);  // Опрос каждую 1 секунду
...
void interface() {
  ...
  //Поля ввода для коректировки температуры
  // crm.input({[Тип], ["ID"], ["Заголовок / значок"], ["Значение по умолчанию"], ["паттерн, смотри в инете"]});
  crm.input({ INPUT_TEXT, "TimeTemp", "&#xe80b; Время показа температуры (в сек)", SHOW_TEMP, "[0-9]{1,7}" });  // Поле ввода значений. Корректный ввод только чисел по регулярному выражению от 1 до 7-х символов
  ...
  // crm.selOpt("{[Заголовок опции]", "[Значение]}");
  // crm.select({["ID"], ["Заголовок / значок"], ["Значение по умолчанию"], "[Флаг обновления интерфейса 1 - да, 0 - нет]" });
  crm.selOpt({ "Ручная настройка", "1" });
  crm.selOpt({ "По расписанию", "2" });
  crm.select({ "select_Regim", "Выбор управления режимом яркости", "1", "1" });  // По умолчанию выбираем "Ручная настройка"

  Regim_bright = crm.var("select_Regim").toInt();  // Так как у нас всего 2 цифры, то смело присваиваем байтовой переменной целое число


  //Режим 1
  // Получить значение из конфига: crm.var(["ID переменной"]). Преобразуем в int и сверяем
  if (Regim_bright == 1) {  // Если выбран второй режим, включаем "Ручная настройка" яркости
  ...
  //Режим 2
  // Получить значение из конфига: crm.var(["ID переменной"]). Преобразуем в int и сверяем
  if (Regim_bright == 2) {  // Если выбран третий режим, включаем режим регулировки яркости "По расписанию"
    ...
    crm.input({ INPUT_TIME, "Start_Night_Mode", "&#xe83b; Начало ночного режима", "20:37:00" });   // Установка времени включения ночного режима
    crm.input({ INPUT_TIME, "End_Night_Mode", "&#xf0f4; Окончание ночного режима", "08:35:00" });  // Установка времени включения дневного режима
    ...
    // Описание пунктов НОЧНОГО режима. Используем маркос "F()", что бы НЕ хранить текст в ОЗУ. Использование значков: &#[код];
    String txt_dark_night = F("Если показания датчика освещённости БОЛЬШЕ установленного значения, то задействуется - Яркость при Вкл. свете.  <br> Иначе задействуется - Яркость в темноте.");

    //Показываем на WEB интерфейсе блок с настройками НОЧНОГО режима
    //crm.output({[Тип], ["ID"], "["Заголовок"]", ["Текст"], ["цвет в HEX формате"]});
    crm.output({ OUTPUT_TEXT, "night_mode", "Настройки ночного режима", txt_dark_night, "#5c8cfd" });

    //Поля ввода значений с датчика освещённости (от 0 до 1024)
    // crm.input({[Тип], ["ID"], ["Заголовок / значок"], ["Значение по умолчанию"]});
    crm.input({ INPUT_TEXT, "meaning_Night_Mode", "&#xf186; Значение освещённости ночью (от 0 до 1024)", "35" });  // Ввод значения порога освещённости на WEB интерфейсе для ночного режима. (значение по умолчанию 35)

    //Ползунок регулировки яркости в темноте
    // crm.range({["ID"], ["Заголовок"], ["Значение по умолчанию"], ["MIN"], ["MAX"], [Шаг], ["Единицы измерения"]});
    crm.range({ "night_dark_brigh", "Яркость в темноте", 1, PERC_MIN_BRIGHT, PERC_MAX_BRIGHT, 1, "%" });  // Задаём яркость ползунком для ночного режима при Выкл. свете (по умолчанию яркость на 1%)

    //Ползунок регулировки яркости при вкл. свете
    // crm.range({["ID"], ["Заголовок"], ["Значение по умолчанию"], ["MIN"], ["MAX"], [Шаг], ["Единицы измерения"]});
    crm.range({ "night_light_brigh", "Яркость при Вкл. свете", 10, PERC_MIN_BRIGHT, PERC_MAX_BRIGHT, 1, "%" });  // Задаём яркость ползунком для ночного режима при Вкл. свете (по умолчанию яркость на 10%)




    //Настройки ДНЕВНОГО режима
    // Описание пунктов НОЧНОГО режима. Используем маркос "F()", что бы НЕ хранить текст в ОЗУ. Использование значков: &#[код];
    String txt_day_night = F("Если показания датчика освещённости БОЛЬШЕ установленного значения, то задействуется - Солнечная яркость.  <br> Иначе задействуется - Пасмурная яркость.");

    // Показываем на WEB интерфейсе блок с настройками ДНЕВНОГО режима
    //crm.output({[Тип], ["ID"], "["Заголовок"]", ["Текст"], ["цвет в HEX формате"]});
    crm.output({ OUTPUT_TEXT, "day_mode", "Настройки дневного режима", txt_day_night, "#ffa500" });

    //Поля ввода значений с датчика освещённости (от 0 до 1024)
    // crm.input({[Тип], ["ID"], ["Заголовок / значок"], ["Значение по умолчанию"]});
    crm.input({ INPUT_TEXT, "meaning_Day_Mode", "&#xf185; Значение освещённости днём (от 0 до 1024)", "997" });  // Ввод значения порога освещённости на WEB интерфейсе для дневного режима. (значение по умолчанию 970)

    //Ползунок регулировки яркости (Пасмурно)
    // crm.range({["ID"], ["Заголовок"], ["Значение по умолчанию"], ["MIN"], ["MAX"], [Шаг], ["Единицы измерения"]});
    crm.range({ "day_murky_brigh", "Пасмурная яркость", 23, PERC_MIN_BRIGHT, PERC_MAX_BRIGHT, 1, "%" });  // Задаём яркость ползунком для дневного режима, когда Пасмурно (по умолчанию яркость на 23%)

    //Ползунок регулировки яркости (Солнечно)
    // crm.range({["ID"], ["Заголовок"], ["Значение по умолчанию"], ["MIN"], ["MAX"], [Шаг], ["Единицы измерения"]});
    crm.range({ "day_solar_brigh", "Солнечная яркость", 50, PERC_MIN_BRIGHT, PERC_MAX_BRIGHT, 1, "%" });  // Задаём яркость ползунком для дневного режима, когда Солнечно (по умолчанию яркость на 50%)
  }
